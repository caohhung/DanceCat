...
class Daemonize(object):
    ...
    def daemonize(self):
        """Daemonize method."""
        if self.get_process_status() is self.PROC_ERROR:
            self._remove_zombie_process()

        if self.get_process_status() is self.PROC_STOP:
        ...
        elif self.get_process_status() is self.PROC_RUNNING:
            pass
        else:
    ...
    def _remove_zombie_process(self):
        ...
        """
        pids = self.get_process_pids()
        if pids:
            for pid in pids:
                psutil.Process(pid).kill()
                print(
                    '[%s] Orphan process with pid=%s killed' % (
